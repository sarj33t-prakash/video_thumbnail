...
//             FloatingActionButton(
//               onPressed: () async {
//                 XFile? video = await mediaPicker.pickVideo(source: ImageSource.gallery);
//                 if(video != null){
//                   setState(() {
//                     _video.text = video.path;
//                   });
...
Future<ThumbnailResult> genThumbnail(ThumbnailRequest request) async {
  Uint8List? bytes;
  final Completer<ThumbnailResult> completer = Completer();
  if (request.thumbnailPath.isNotEmpty) {
    final thumbnailPath = await VideoThumbnail.thumbnailFile(
      video: request.video,
      headers: {
        "USERHEADER1": "user defined header1",
        ...
        if (snapshot.hasData) {
          ...
          return Column(
            ...
            children: [
              Center(
                child: Text(
                  "Image ${widget.thumbnailRequest.thumbnailPath.isEmpty ? 'data size' : 'file size'}: $dataSize, width:$width, height:$height",
                ),
              ),
              Container(
                color: Colors.grey,
...
class _DemoHomeState extends State<DemoHome> {
  ...
  void initState() {
    super.initState();
    getTemporaryDirectory().then((dir) => _tempDir = dir.path);
  }
    ...
    return Scaffold(
          ...
          Expanded(
            child: Container(
              ...
              child: Scrollbar(
                child: ListView(
                  shrinkWrap: true,
                  children: [_futureImage ?? const SizedBox()],
                ),
              ),
            ),
          ),
      ...
      floatingActionButton: Row(
        ...
        children: [
          FloatingActionButton(
            onPressed: () async {
              final video = await mediaPicker.pickVideo(source: ImageSource.camera);
              if (video != null) {
                setState(() {
                  _video.text = video.path;
                });
